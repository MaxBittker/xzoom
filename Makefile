...
# operating system:  Linux 4.19.91-1-MANJARO x86_64 [ELF] (4.19.91)
# libc:	(6.30.0)
# binutils:	(233)
...
.SUFFIXES: .cc
    ...
    CXXDEBUGFLAGS =  -fno-strength-reduce -fno-strict-aliasing
CXXDEPENDINCLUDES =
 CXXEXTRA_DEFINES =
CXXEXTRA_INCLUDES =
  ...
  EXTRA_LIBRARIES =
   ...
   PARALLELMFLAGS =
    ...
    INSTKMEMFLAGS = -m 4711
      ...
      CDEBUGFLAGS =  -fno-strength-reduce -fno-strict-aliasing
        CCOPTIONS =
...
#
# Valid compile time options:
# -DFRAME: source area is marked with a rectangular frame.
# -DXSHM:  use X11 shared memory extension.
# -DTIMER: count time between window updates (just for testing).
# -DNO_USLEEP: for system that do not have the usleep function
# -DBCOPY: use bcopy() instead of memmove()

# DEFINES = -DFRAME -DXSHM -DTIMER -DNO_USLEEP
...
$(NAME).$(MANNEWSUFFIX): $(NAME).$(MANSRCSUFFIX)
	...
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   ...
	   $(LN) `basename $(NAME).$(MANSRCSUFFIX)` `basename $@`; \
	     ...
	     < $(NAME).$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) $(NAME).$(MANNEWSUFFIX)

install.man:: $(NAME).$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) $(NAME).$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/$(NAME).$(MANSUFFIX)
